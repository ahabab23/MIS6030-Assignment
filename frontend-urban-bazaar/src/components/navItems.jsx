    ...
    if (searchQuery.trim()) {
      try {
        const response = await fetch(
          `http://127.0.0.1:5001/products?q=${encodeURIComponent(searchQuery)}`
        );
      ...
      <div className="bg-blue-200 border-b border-gray-300">
        <div className="flex h-8 items-center justify-end px-4 sm:px-6 lg:px-8">
          <div className="flex space-x-4">
            <Link
              to="/signup"
              className="text-gray-700 font-medium text-sm hover:text-blue-700"
            >
              Join UrbanBazaar
            </Link>
            <Link
        ...
        <div className="flex flex-col sm:flex-row justify-between h-auto sm:h-16 items-center px-4 sm:px-6 lg:px-8 py-4 sm:py-0">
          ...
          <div className="flex items-center gap-5 w-full sm:w-auto justify-center mt-2 sm:mt-0">
            <form
              ...
              className="flex items-center mb-4 sm:mb-0 w-full sm:w-auto"
            >
              <input
                type="text"
                placeholder="Search..."
          ...
          <div className="flex items-center space-x-4 sm:space-x-6">
            ...
            <Link
              to="/wishlist"
              className="text-gray-700 hover:text-blue-700 flex items-center relative"
            >
              <FontAwesomeIcon
                icon={faHeart}
                className="text-lg border border-gray-400 rounded-full p-1"
              />
              {wishlist.length > 0 && (
